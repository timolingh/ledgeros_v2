...
from apps.accounting.selectors import account_balance, trial_balance
...
def balances_by_code() -> dict[str, str]:
    return {row["account_code"]: row["balance"] for row in trial_balance()}


@pytest.mark.django_db
def test_trial_balance_uses_posted_ledger_activity(accounting_ready):
    ...
    assert account_balance(revenue) == Decimal("100.00")
    assert balances_by_code() == {"1000": "100.00", "4000": "100.00"}

    reverse_journal_entry(entry=entry, reversal_date=date(2026, 5, 2))

    assert account_balance(cash) == Decimal("0.00")
    assert account_balance(revenue) == Decimal("0.00")
    assert balances_by_code() == {"1000": "0.00", "4000": "0.00"}
