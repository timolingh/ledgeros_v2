...
- Tax summary reporting that surfaces configured tax liability accounts and balances
...
- Cash-basis P&L recognizes income and expense when payments are applied, not when bank deposits or reconciliation occur.
- Bank deposits from `Undeposited Funds` do not create additional revenue or expense recognition in Epic 4.
...
- Period summaries report only journal activity dated inside the selected period, not cumulative as-of-end balances.
- Tax support in Epic 4 covers tax code configuration and tax liability reporting only.
- Epic 4 does not calculate sales tax on AR invoices, generate tax entries from invoice lines, or create a full sales/use tax accounting engine.
...
| Tax code configuration | Epic 4 / PRD TAX-001, TAX-005 | Implemented | `apps/accounting/models/reporting.py`, `apps/accounting/services/reporting.py` | `apps/accounting/tests/test_reporting_service.py` |
...
| Sales tax on AR invoices | PRD TAX-002 / TAX-004 | Deferred | N/A | Not implemented in Epic 4 |
| Sales/use tax liability account posting from invoices | PRD TAX-002 / TAX-004 | Deferred | N/A | Not implemented in Epic 4 |
| US/California tax-specific liability or expense structure | PRD TAX-005 | Deferred | N/A | TaxCode jurisdictions exist, but no special posting structure is created yet |
| Tax reporting lines in financial statements | Epic 4 / PRD TAX-003 | Deferred | N/A | Epic 4 exposes a tax summary API instead of embedded statement lines |
...
Summarize a period using only in-period activity:

```bash
docker compose run --rm -T web python manage.py shell <<'PY'
from datetime import date
from apps.accounting.services import summarize_period
from apps.accounting.models import AccountingPeriod

period = AccountingPeriod.objects.get(name="May 2032 Period Summary Test")
print(summarize_period(period=period))
PY
```
...
- Period summaries are intentionally period-only. They count posted journal entries and debit/credit totals dated within the selected period window.
